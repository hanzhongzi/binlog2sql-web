...
from binlog2sql_web.source_code.binlog2sql_util import  concat_sql_from_binlog_event, create_unique_file, temp_open, \
    reversed_lines, is_dml_event, event_type
from datetime import  timezone, timedelta
tz_east_8 = timezone(timedelta(hours=8))
#binlog2sql_web.source_code.
class Binlog2sql(object):
    ...
    def process_binlog(self):
        ...
        stream = BinLogStreamReader(connection_settings=self.conn_setting, server_id=self.server_id,
                                    ...
                                    only_tables=self.only_tables, resume_stream=True, blocking=True) #blocking=True
        ...
        with temp_open(tmp_file, "w") as f_tmp, self.connection as cursor:
            for binlog_event in stream:
                ...
                if isinstance(binlog_event, QueryEvent) and not self.only_dml:
                    ...
                    if sql and "GRANT" not in sql:
                        #print(sql)
                      
                        output_lines.append(sql)  # 使用 append 添加到列表中
